...
# --- palette source ---
# chrysaki-core.sh is the generated core token file, synced from the
# chrysaki-core dependency. integration.sh layers statusline mappings on it.
_CC_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" 2>/dev/null && pwd)"
if [ -f "$_CC_DIR/palette/chrysaki-core.sh" ]; then
  . "$_CC_DIR/palette/chrysaki-core.sh"
  . "$_CC_DIR/palette/integration.sh"
fi

# hex_fg: convert '#rrggbb' to a truecolor foreground escape.
hex_fg() {
  local h="${1#\#}"
...
if [ "$NO_COLOUR" -eq 1 ]; then
  ...
  # Colours come from palette/chrysaki-core.sh plus palette/integration.sh.
  # Each fallback hex keeps the statusline working without the palette files.

  # Identity colours
  C_BLONDE_LT=$(hex_fg "${CC_BLONDE_LT:-#d0b850}")     # unsynced commits
  C_TEAL=$(hex_fg "${CC_TEAL:-#1e8898}")               # ctx (normal), issues
  C_EMERALD_LT=$(hex_fg "${CC_EMERALD_LT:-#1a8a6a}")   # 5h normal, inbox
  C_ORANGE="\033[38;5;208m"                             # terminal orange -- ctx warning

  # Text hierarchy
  C_SEC=$(hex_fg "${CC_SEC:-#a0a4b8}")                 # 7d normal
  C_MUTED=$(hex_fg "${CC_MUTED:-#6a6e82}")             # separators, reset timers

  # Alert thresholds
  C_WARN=$(hex_fg "${CC_WARN:-#b8a038}")               # >=50% warning
  C_ERROR=$(hex_fg "${CC_ERROR:-#c04050}")             # >=75%/128k critical

  # Bar empty-position colour
  C_HEX_EMPTY=$(hex_fg "${CC_HEX_EMPTY:-#40465a}")     # dim background positions

  # Git change colours
  C_GREEN=$(hex_fg "${CC_GREEN:-#50b450}")             # insertions
  C_RED=$(hex_fg "${CC_RED:-#c05050}")                 # deletions
fi
...
# Embedders can provide their actual render width. This is reliable for Pi, whose
# footer is rendered without a controlling TTY; fall back to TTY/COLUMNS detection
# for standalone Claude Code use.
COLS="${CHRYSAKI_COLUMNS:-${COLS:-${COLUMNS:-$(tput cols 2>/dev/null || echo 80)}}}"
