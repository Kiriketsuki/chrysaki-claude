...
Chrysaki theme integration for Claude Code. A four-line status bar in the Chrysaki Geometric Dashboard design, plus a stats fetcher and a session cost exporter.

Colours come from [chrysaki-core](https://github.com/Kiriketsuki/chrysaki-core), pinned as a git dependency. `palette/chrysaki-core.sh` is the committed copy of the generated core token file. `palette/integration.sh` maps core tokens to statusline roles and holds the integration-local variants. The statusline sources both at render time, so node is only a build-time dependency.

## Palette workflow

1. Bump the `@kiriketsuki/chrysaki-core` tag in `package.json`.
2. Run `npm install && npm run sync-palette`.
3. Commit the refreshed `palette/chrysaki-core.sh`.

`npm run check-palette` verifies the committed copy matches the dependency.
