...
_export_one_cache() {
  ...
  # A partially written or older cache can leave this field blank/non-numeric.
  # Treat it as zero so a Stop hook never emits a printf error or blocks cleanup.
  if ! [[ "$_raw_usd" =~ ^[0-9]+([.][0-9]+)?$ ]]; then
    _raw_usd="0"
  fi
